...
public class LinkSelectorProvider {
    ...
    private static final String defaultLinkSelector = "a[href]:not([href~=(?i).*(\\.(css|js|bmp|gif|jpe?g|png|tiff?|mid|mp2|mp3|mp4|wav|avi|mov|mpeg|ram|m4v|pdf" +
            "|rm|smil|wmv|swf|wma|zip|rar|gz|csv|xls|ppt|doc|docx|exe|dmg|midi|mid|qt|txt|ram|json))$)" +
            ":not([href~=(?i)^#])";

    static {
        final String wikipediaSelector = "#content :not(.mw-editsection) > a[href]:not([href~=(?i).*(\\." +
